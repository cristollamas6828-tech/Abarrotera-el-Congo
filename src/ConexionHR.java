...
// En Formulario.java
    // BORRA EL MÉTODO ANTERIOR Y PEGA ESTE COMPLETO:
    //--------------------------------------------------------------------------
    //-------- Enviar datos (Insertar) - VERSIÓN MEJORADA PARA DEBUG
    //--------------------------------------------------------------------------
    public boolean enviarFormulario(java.util.Map<String, String> datos) {
        try {
            URL urlv = new URL(this.url);
            HttpURLConnection con = (HttpURLConnection) urlv.openConnection();
            con.setRequestMethod("POST");
            con.setDoOutput(true);
            con.setRequestProperty("Content-Type", "application/x-www-form-urlencoded");
            // A veces los hostings gratuitos bloquean User-Agents de Java, fingimos ser un navegador:
            con.setRequestProperty("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64)");

            if (!datos.containsKey("key")) {
                datos.put("key", "secret");
            }

            StringBuilder postData = new StringBuilder();
            for (java.util.Map.Entry<String, String> entrada : datos.entrySet()) {
                if (postData.length() != 0) {
                    postData.append("&");
                }
                postData.append(URLEncoder.encode(entrada.getKey(), "UTF-8"));
                postData.append("=");
                postData.append(URLEncoder.encode(entrada.getValue(), "UTF-8"));
            }

            byte[] postBytes = postData.toString().getBytes("UTF-8");
            con.getOutputStream().write(postBytes);

            // Leer TODA la respuesta (no solo la primera línea)
            ...
            while ((linea = rd.readLine()) != null) {
                respuestaCompleta.append(linea).append("\n");
            }
            rd.close();

            String respuesta = respuestaCompleta.toString().trim();

            // Verificar si es OK
            if (con.getResponseCode() == 200 && respuesta.equals("OK")) {
                return true;
            } else {
               

                // Mostrar un mensaje corto al usuario, pero avisar que mire la consola
                javax.swing.JOptionPane.showMessageDialog(null, "El servidor respondió con un error (ver Output).\nPosible causa: " + respuesta.split("\n")[0]);
                return false;
            }
...
//                doc.add(image);
    ...
    public String toSQL(java.util.Date fecha) {
        ...
        // Esto garantiza el formato AAAA-MM-DD correcto (ej: 2025-01-01)
...
//--------------------------------------------------------------------------
    ...
    public java.util.Date toDate(String fecha) {
        // 1. Validar si la fecha es nula o vacía para evitar el error
        if (fecha == null || fecha.trim().isEmpty()) {
            return null; // Devuelve null para que el JDateChooser se quede vacío
        }

        try {
            String partes[] = fecha.split("-");

            // 2. Validar que tenga el formato correcto (año-mes-dia)
            if (partes.length < 3) {
                return null;
            }

            int yy = Integer.parseInt(partes[0]);
            int mm = Integer.parseInt(partes[1]); // Nota: En el constructor Date, el mes suele ser 0-11.
            int dd = Integer.parseInt(partes[2]);

            // Se resta 1900 al año y 1 al mes porque el constructor Date(anio, mes, dia) es antiguo
            // y cuenta los años desde 1900 y los meses desde 0.
    ...
    public int insertar(String tabla, String[] valores) {
        ...
        for (int k = 0; k < valores.length; k++) {
            if (valores[k] == null || valores[k].equals("")) {
                sql += "NULL";
            } else {
                sql += "'" + valores[k] + "'";
            }
    ...
    //-------------------------------------------------------
    //-------------------------------------------------------
    //---Actualizar un registro en una tabla (CORREGIDO)
    //-------------------------------------------------------
