...
// En Formulario.java
    ...
    public boolean enviarFormulario(java.util.Map<String, String> datos) {
        try {
            // 1. Definir la URL y CREAR la variable 'con' (IMPORTANTE: ESTO VA PRIMERO)
            URL urlv = new URL(this.url);
            HttpURLConnection con = (HttpURLConnection) urlv.openConnection(); // <--- Aquí nace 'con'

            // 2. Ahora sí podemos configurar 'con' (porque ya existe)
            con.setRequestMethod("POST");
            con.setDoOutput(true);
            con.setRequestProperty("Content-Type", "application/x-www-form-urlencoded");
            con.setRequestProperty("User-Agent", "Mozilla/5.0");

            // 3. Agregar la clave de seguridad si falta
            ...
            if (responseCode == 200 && respuesta != null && respuesta.trim().equals("OK")) {
                return true;
            } else {
                System.out.println("ERROR DEL SERVIDOR (" + responseCode + "): " + respuesta);
                javax.swing.JOptionPane.showMessageDialog(null, "Error: " + respuesta);
                return false;
            }
...
//                doc.add(image);
    ...
    public java.util.Date toDate(String fecha) {
        // Definir el formato que viene de MySQL
        java.text.SimpleDateFormat sdf = new java.text.SimpleDateFormat("yyyy-MM-dd");
        try {
            // Intenta convertir la cadena (ej. "2025-11-25") a un objeto Date
            return sdf.parse(fecha);
        } catch (java.text.ParseException e) {
            System.out.println("Error al parsear la fecha: " + fecha);
            e.printStackTrace();
            return null; // Devuelve null si falla la conversión
        }
    }
    ...
    public int borrar(String tabla, String[] valores) {
        ...
        try {
            ...
            // CORRECCIÓN: Agregar verificación de resultado
            if (resultado != null && resultado.contains("AFFECTED ROWS")) {
                correcta = 1;
            }
